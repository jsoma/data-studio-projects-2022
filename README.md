...
* Minimum font size should be 12px, enlarge text in Illustrator
    ...
    * Text `20` is too small at 8px
    * Text `30` is too small at 8px
    * Text `40` is too small at 8px
    * Text `50` is too small at 8px
    * Text `60` is too small at 8px
    * Text `Anderson Cooper` is too small at 8px
    * *and 29 more*
* Overlapping elements in ai2html, check [the overflow video](https://www.youtube.com/watch?v=6vHsnjTp3_w) or make a smaller size
   * Text `Republicans` overlaps with `Democrats` at screen width 400
   * Text `Republicans` overlaps with `​ US Adult Citizen` at screen width 400
   * Text `Anderson Cooper` overlaps with `CNN` at screen width 400
   * Text `Rachel Maddow` overlaps with `MSNBC` at screen width 400
   * Text `G. Stephanopoulos` overlaps with `ABC` at screen width 400
   * Text `Wolf Blitzer` overlaps with `CNN` at screen width 400
   * Text `Gayle King` overlaps with `CBS` at screen width 400
   * *and 44 more*
